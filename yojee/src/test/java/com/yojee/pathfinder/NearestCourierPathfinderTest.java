...
class NearestCourierPathfinderTest {
    ...
    void findPathsNoCourier() {
        try {
            new NearestCourierPathfinder().findPaths(null, new Location(0,0), 0);
    ...
    void findPathsNoDistributionPoint() {
        try {
            new NearestCourierPathfinder().findPaths(null, null, 15);
            fail("Expected IllegalArgumentException");
        } catch (IllegalArgumentException e) {
            //all good
        }
    ...
    void findPathNullLocations() {
        List<Path> paths = new NearestCourierPathfinder().findPaths(null, DISTRIBUTION_POINT, 1);
        assertNotNull(paths);
    ...
    void findPathNoLocations() {
        ...
        List<Path> paths = new NearestCourierPathfinder().findPaths(locations, DISTRIBUTION_POINT, 1);
        assertNotNull(paths);
        assertEquals(1, paths.size());
        assertEquals(1, paths.get(0).getLocations().size());
        assertEquals(DISTRIBUTION_POINT, paths.get(0).getLocations().get(0));
    ...
    void findPathOneLocationTwoCouriers() {
        List<Location> locations = new ArrayList<>();
        locations.add(NEW_YORK);
        List<Path> paths = new NearestCourierPathfinder().findPaths(locations, DISTRIBUTION_POINT, 2);
        assertNotNull(paths);
    ...
    void findPathTwoLocationsOneCourier() {
        ...
        assertEquals(DISTRIBUTION_POINT, paths.get(0).getLocations().get(0));
        assertEquals(NEW_YORK, paths.get(0).getLocations().get(1));
        assertEquals(LOS_ANGELES, paths.get(0).getLocations().get(2));
    }
