...
class NearestNeighborPathfinderTest {
    ...
    void findPathsNoCourier() {
        try {
            new NearestNeighborPathfinder().findPaths(null, new Location(0,0), 0);
    ...
    void findPathsNoDistributionPoint() {
        try {
            new NearestNeighborPathfinder().findPaths(null, null, 15);
            fail("Expected IllegalArgumentException");
        } catch (IllegalArgumentException e) {
            //all good
        }
    ...
    void findPathNullLocations() {
        List<Path> paths = new NearestNeighborPathfinder().findPaths(null, DISTRIBUTION_POINT, 1);
        assertNotNull(paths);
    ...
    void findPathNoLocations() {
        ...
        List<Path> paths = new NearestNeighborPathfinder().findPaths(locations, DISTRIBUTION_POINT, 1);
        assertNotNull(paths);
        assertEquals(1, paths.size());
        assertEquals(1, paths.get(0).getLocations().size());
        assertEquals(DISTRIBUTION_POINT, paths.get(0).getLocations().get(0));
    ...
    void findPathOneLocationTwoCouriers() {
        List<Location> locations = new ArrayList<>();
        locations.add(NEW_YORK);
        List<Path> paths = new NearestNeighborPathfinder().findPaths(locations, DISTRIBUTION_POINT, 2);
        assertNotNull(paths);
    ...
    void findPathTwoLocationsTwoCouriers() {
        ...
        assertEquals(2, paths.size());
        assertEquals(2, paths.get(0).getLocations().size());
        assertEquals(2, paths.get(1).getLocations().size());
        assertEquals(DISTRIBUTION_POINT, paths.get(0).getLocations().get(0));
        assertEquals(LOS_ANGELES, paths.get(0).getLocations().get(1));
        assertEquals(DISTRIBUTION_POINT, paths.get(1).getLocations().get(0));
        assertEquals(NEW_YORK, paths.get(1).getLocations().get(1));
    }
    ...
    void findNearestNeighborTest(){
        ...
        locations.add(NEW_YORK);
        locations.add(LOS_ANGELES);
        assertEquals(LOS_ANGELES, new NearestNeighborPathfinder().findNearestNeighbor(locations, DISTRIBUTION_POINT));
    }
}